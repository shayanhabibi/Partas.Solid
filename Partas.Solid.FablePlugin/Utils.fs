...
type AstUtils =
    ...
    static member inline ValueArray(exprs: Expr list) =
        Expr.Value(
            ValueKind.NewArray(
                NewArrayKind.ArrayValues(exprs),
                any, ArrayKind.MutableArray
                ), range
            )
    ...
    static member inline Unit =
        Expr.Value(ValueKind.UnitConstant, range)
    /// Creates a null constant expression
    static member inline Null =
        Expr.Value(ValueKind.Null any, range)
    ...
    static member inline Object(pairs: (string * Expr) list) =
        Expr.Value(ValueKind.NewAnonymousRecord(List.map snd pairs, List.map fst pairs |> List.toArray, List.map (fun _ -> any) pairs, false), range)
    ...
    static member inline IdentExpr(name: string, ?isThisArg: bool, ?isMutable: bool, ?isCompilerGenerated: bool) =
        let isMutable,isCompilerGenerated,isThisArg =
            defaultArg isMutable true,
            defaultArg isCompilerGenerated true,
            defaultArg isThisArg false
        Expr.IdentExpr({ Type = any; IsCompilerGenerated = isCompilerGenerated; IsMutable = isMutable; IsThisArgument = isThisArg; Name = name; Range = range })
    /// Creates an emit statement macro function with the given macro
    static member inline EmitStatement(macro: string) = fun callInfo ->
        Expr.Emit({ CallInfo = callInfo; IsStatement = true; Macro = macro }, any, range)
    /// Creates an emit statement macro with the given macro and args
    static member inline EmitStatement(macro, callInfo) =
        callInfo |> AstUtils.EmitStatement(macro)
    /// Creates an emit macro function with the given macro
    static member inline Emit(macro: string) = fun callInfo ->
        Expr.Emit({ CallInfo = callInfo; IsStatement = false; Macro = macro }, any, range)
    /// Creates an emit macro expression with the given macro and args
    static member inline Emit(macro,callInfo) =
        callInfo |> AstUtils.Emit(macro)
    ...
    static member inline EmitPure(js: string, ?isStatement: bool) =
        ...
        Expr.Emit({
            CallInfo = { Args = []; GenericArgs = []; MemberRef = None; SignatureArgTypes = []; Tags = []; ThisArg = None }
            IsStatement = isStatement
            Macro = js
        }, any, range)
    /// Creates a JS statement expression
    static member inline EmitPureStatement(macro) = AstUtils.EmitPure(macro, true)
    ...
    static member inline FieldInfo(name: string, ?hasSideEffects: bool) =
        ...
        { Name = name; IsMutable = hasSideEffects; Tags = []; FieldType = None; MaybeCalculated = hasSideEffects }
    ...
    static member inline GetProp(identExpr: Ident, field: string) =
        Expr.Get(Expr.IdentExpr { identExpr with IsThisArgument = true; IsCompilerGenerated = true }, GetKind.ExprGet(AstUtils.Value(field)), any, range)
    ...
    static member inline Ident(name: string, ?isThisArg: bool, ?typ: Type, ?isMutable: bool, ?isCompilerGenerated: bool, ?range: SourceLocation) =
        ...
        { Name = name; Type = typ; IsCompilerGenerated = isCompilerGenerated; IsMutable = isMutable; IsThisArgument = isThisArg; Range = range }
    static member inline CallInfo(?thisArg: Expr, ?args: Expr list, ?genArgs: Type list, ?sigArgTypes: Type list, ?memberRef: MemberRef, ?isCons: bool, ?tag: string) =
        CallInfo.Create(?thisArg = thisArg, ?args = args, ?genArgs = genArgs, ?sigArgTypes = sigArgTypes, ?memberRef = memberRef, ?isCons = isCons, ?tag = tag)
    static member inline TypeCast(expr: Expr, ?typ: Type) =
        TypeCast (expr, typ |> Option.defaultValue any)
type JsxUtils =
    /// The fable jsx element type pre-referenced
    static member inline ElementType = Type.DeclaredType({ FullName = "Fable.Core.JSX.Element"; Path = EntityPath.CoreAssemblyName "Fable.Core" }, [])
    /// The fable jsx element create func pre-referenced
    static member inline CreateElementImportExpr = AstUtils.Import("create", "@fable-org/fable-library-js/JSX.js")
    ...
    static member inline CreateElement(tagNameExpr: Expr, properties: (string * Expr) list, children: Expr list) =
        let internalCollection =
            JsxUtils.ChildrenProp(children) :: (properties |> List.map JsxUtils.Prop)
            |> AstUtils.Flatten
        Call(JsxUtils.CreateElementImportExpr, { ThisArg = None; Args = [ tagNameExpr; internalCollection ]; SignatureArgTypes = []; GenericArgs = []; MemberRef = None; Tags = [ "jsx" ] }, JsxUtils.ElementType, range)
...
module Patterns =
    ...
    let rec (|ExprTypeCastDrill|): Expr -> Expr = function
    ...
    let (|EntityRefHasAttribute|_|) (pluginHelper: PluginHelper) (attrName: string): EntityRef -> Attribute option =
        pluginHelper.GetEntity >> _.Attributes >> Seq.tryFind ( _.Entity >> _.FullName >> (=) attrName)
    ...
    let (|TypeHasAttribute|_|) (pluginHelper: PluginHelper) (attrName: string): Type -> Attribute option =
        function
        | GetDeclaredType (Type.DeclaredType(EntityRefHasAttribute pluginHelper attrName attr, _)) ->
            Some attr
        | _ -> None
...
type StringUtils =
    ...
    static member TrimReservedIdentifiers =
        ...
        | s when s.Length > 2 && s.Substring(s.Length - 2).StartsWith '`' ->
            s.Substring(0, s.Length - 2) |> StringUtils.TrimReservedIdentifiers
        | s -> s
