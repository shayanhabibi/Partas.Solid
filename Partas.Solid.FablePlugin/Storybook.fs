    ...
    type AstUtils with
        ...
        static member Table
            ...
            [ if category.IsSome then
              ...
              if
                  defaultValueDetail.IsSome
                  || defaultValueSummary.IsSome
              then
        ...
        static member ControlType
            (?typ: string, ?accept: string, ?labels: (string * string) list, ?max: Expr, ?min: Expr, ?presetColors: string list, ?step: Expr)
            =
            let doNotRender =
                typ
                |> Option.exists ((=) "false")
        ...
        static member ArgType
            (?control: Expr, ?description: string, ?conditional: Expr, ?mapping: Expr, ?name: string, ?options: Expr list, ?table: Expr, ?typ: Expr)
            =
            [ if control.IsSome then
                  "control", control.Value
              if description.IsSome then
                  "description", AstUtils.Value (description.Value)
              if conditional.IsSome then
                  "conditional", conditional.Value
              if mapping.IsSome then
                  "mapping", mapping.Value
              if name.IsSome then
                  "name", AstUtils.Value name.Value
              if options.IsSome then
                  "options", AstUtils.ValueArray options.Value
        ...
        static member Meta
            ...
            [ "component", comp
              if subComps.IsSome then
                  "subcomponents", AstUtils.ValueArray subComps.Value
              if args.IsSome then
                  "args", args.Value
              if argTypes.IsSome then
                  "argTypes", argTypes.Value
              if render.IsSome then
                  "render", render.Value
              if stories.IsSome then
                  "stories",
                  stories.Value
                  |> List.map AstUtils.Value
                  |> AstUtils.ValueArray
...
module internal rec StorybookTypeRecursion =
    ...
    let rec collectEntityMembers (ctx: PluginContext) (entity: Entity) =
        ...
        let entityMembers = getEntityMembers ctx entity
    ...
    let getVariants (ctx: PluginContext) (expr: Expr) =
        ...
        let processRawVariantExpr (RawVariantExpr (name, expr)) =
            let predicate =
                function
                | Set _ -> true
                | Call (callee = Import (info = { Kind = ImportKind.MemberImport (MemberRef (info = { CompiledName = compiledName })) })) when
                    compiledName
                    |> _.Split('.')
                    |> Array.last
                    |> _.StartsWith("set_")
            ...
            |> List.choose (function
                ...
                | Call (
                    ...
                    let prop =
                        ...
                        |> Array.last
                        |> function
                            | StartsWithTrimmed "set_" value -> value
                            | _ -> failwith "Unreachable"
            ...
            |> fun args ->
                let args =
                    args
                    |> List.map (function
                        | "children", expr -> "children", AST.transform ctx expr
                        | prop, expr -> prop, expr)
...
module internal StorybookAST =
    let getComponentExprFromEntity (ctx: PluginContext) (entity: Entity) =
        ...
        if
            entRef.SourcePath
            |> Option.exists ((=) ctx.Helper.CurrentFile)
        then
    ...
    let createUnionExprs (ctx: PluginContext) (entity: Entity) =
        let cases =
            if not entity.IsFSharpUnion then
                None
            else
                Some entity.UnionCases

        cases
        |> Option.map (List.mapi (fun idx case -> Expr.Value (ValueKind.NewUnion ([], idx, entity.Ref, []), None)))
    ...
    let createMeta (ctx: PluginContext) (memberDecl: MemberDecl) (expr: Expr) =
        ...
        let entity =
            ...
            match typ with
            | StorybookTypeRecursion.GetGenericArg ctx entity -> entity
        ...
        let entityXmlDocs =
            ...
            |> Option.bind (
                ...
                >> fun docs ->
                    if
                        docs
                        |> Option.exists String.IsNullOrWhiteSpace
                    then
                        None
                    else
                        docs
            ...
            |> Option.orElse (
                ...
                |> Seq.tryFind (
                    ...
                    >> Seq.exists (
                        _.Entity.FullName
                        >> (=) "Partas.Solid.SolidTypeComponent"
                    )
                )
                |> Option.bind (
                    _.XmlDoc
        ...
        let fieldData =
            ...
            |> List.map (fun prop ->
                ...
                let getElementValue value =
                    docs
                    |> Option.bind (extractElementValue value)
                ...
                let docData =
                    let controlTypeAttribute =
                        ...
                        |> Option.bind (
                            ...
                            >> Seq.map (
                                _.XPathEvaluate("string(@controlType)")
                                >> unbox<string>
                            )
                            >> Seq.tryHead
                        )
                ...
                let controlTypeDefault (value: ControlType) =
                    docData.Storybook.ControlType
                    |> Option.defaultValue (value.ToString ())

                let rec makeArgType typ =
                    match typ with
                    | Any ->
                        let control = AstUtils.ControlType (typ = controlTypeDefault ControlType.Object)
                    ...
                    | Type.Boolean ->
                        let control = AstUtils.ControlType (typ = controlTypeDefault ControlType.Boolean)
                    ...
                    | Char ->
                        let control = AstUtils.ControlType (typ = controlTypeDefault ControlType.Text)
                    ...
                    | String ->
                        ...
                        | Some values ->
                            ...
                            AstUtils.ArgType (
                                control = control,
                                table = AstUtils.Table (typSummary = "[<StringEnum>]", ?defaultValueSummary = defaultValue),
                                options =
                                    (values
                                     |> List.map AstUtils.Value),
                                ?description = description
                    ...
                    | Regex ->
                        ...
                        AstUtils.ArgType (
                            control = control,
                            table = AstUtils.Table (typSummary = "Regex", typDetail = "Expects valid regex", ?defaultValueSummary = defaultValue),
                            ?description = description
                        )
                    // todo - support enums
                    | Type.Number (kind, info) ->
                        let control =
                            ...
                            | UInt64 -> AstUtils.ControlType (typ = ControlType.Number, min = AstUtils.Value (0), step = AstUtils.Value (1))
                            | UNativeInt
                            | UInt128 -> AstUtils.ControlType (typ = ControlType.Number, step = AstUtils.Value (1), min = AstUtils.Value (0))
                            | Int128
                            | NativeInt
                            | Int64
                            | BigInt -> AstUtils.ControlType (typ = ControlType.Number, step = AstUtils.Value (1))
                            | Float16
                            | Float32 ->
                                AstUtils.ControlType (
                                    typ = ControlType.Number,
                            ...
                            | Float64
                            | Decimal -> AstUtils.ControlType (typ = ControlType.Number)
                            | Int32 ->
                                AstUtils.ControlType (
                                    typ = ControlType.Number,
                    ...
                    | Nullable (genArg, _)
                    | Option (genArg, _) -> makeArgType genArg
                    | LambdaType _
                    | DelegateType _ ->
                    ...
                    | GenericParam (name, isMeasure, constraints) ->
                        ...
                        AstUtils.ArgType (
                            control = control,
                            table = AstUtils.Table (typSummary = $"GenericParam {name}", ?defaultValueSummary = defaultValue),
                            ?description = description
                        )
                    | DeclaredType (ref, genArgs) ->
                        ...
                        if entity.IsFSharpRecord then
                            let typName =
                                entity.FSharpFields
                                |> List.map (fun field -> $"{field.Name}: {field.FieldType}")
                                |> String.concat ", "
                                |> sprintf "{ %s }"
                        ...
                        // TODO
                            ...
                            AstUtils.ArgType (
                                control = control,
                                table =
                                    AstUtils.Table (typSummary = ref.DisplayName, ?typDetail = ref.SourcePath, ?defaultValueSummary = defaultValue),
                                ?description = description
                            )
                ...
                |> fun expr ->
                    ...
                    { Name = name
                      Arg =
                        ...
                        |> Option.orElse (
                            if
                                (prop.Type.IsDelegateType
                                 || prop.Type.IsLambdaType)
                                && (prop.Name.StartsWith "on"
                                    || docData.Storybook.Spy)
                            then
                                Some
                                <| AstUtils.Call (AstUtils.Import ("fn", "storybook/test"), AstUtils.CallInfo ())
                            else
                                None
                        )
                      ArgType = expr
        ...
        let compExpr =
            AstUtils.Emit (
                [ $"$0\n }};\n\nexport default {memberDecl.Name};\n"
                  yield!
                      (variantCombinations
                       |> List.mapi (fun idx (name, _) -> $"export const {name} = ${idx + 2}"))
                  closeLastTemplate ]
                |> String.concat "\n",
                AstUtils.CallInfo (
                    args =
        ...
        [ "args",
          ...
          |> List.choose (fun data ->
              ...
              |> function
                  | Some (_, expr) -> Some (data.Name, expr)
                  | None when data.Arg.IsSome -> Some (data.Name, data.Arg.Value)
                  | _ -> None)
          |> AstUtils.Object
          "argTypes",
          AstUtils.Object (
        ...
        |> fun expr ->
            ...
            | { XmlDoc = None | Some "" } ->
                { memberDecl with
                    XmlDoc = entityXmlDocs }
            | _ -> memberDecl
            |> function
                | memberDecl -> { memberDecl with Body = expr }

    [<return: Struct>]
    let rec (|RootExpr|_|) ctx =
        function
        | Call (callee = (Import (info = { Selector = StartsWith "StorybookExtensions_Run" }))) as expr -> ValueSome expr
        | _ -> ValueNone
    ...
    let transform (ctx: PluginContext) (memberDecl: MemberDecl) =
        ...
        findAndDiscardElse predicate expr
        |> function
            | [ expr ] -> createMeta ctx memberDecl expr
...
type PartasStorybookAttribute(compFlags: int) =
    ...
    override this.TransformCall(_, _, expr) = expr
