                        ...
                        info = CallInfo.Create( args = [
                            Value(
                                kind = ValueKind.NewAnonymousRecord(
                                    ...
                                    fieldNames = (values |> List.map (fst >> trimReservedIdentifiers) |> List.toArray),
                                    genArgs = [],
                                    isStruct = false
                                ),
                                range = None
...
module internal rec AST =
    ...
    module Attributes =
        ...
        let private (|AttributeExpression|_|) (context: PluginContext): Expr -> PropInfo option =
            ...
            | Call(Import(importInfo, _, _), callInfo, _, _) ->
                ...
                | ImportKind.MemberImport(MemberRef(entity, memberRefInfo)) when
                    ...
                    | "spread", { Args = [ _; IdentExpr({ Name = "props" }) ] } -> Some("__SPREAD_PROPERTY__", Helpers.Baked.spreadPropertyExpression)
                    ...
                    | _ when memberRefInfo.CompiledName.IndexOf("set_") >= 0 ->
                        ...
                        | Delegate(args, expr, name, tags) -> (propName, Delegate(args, transform expr context, name, tags))
        ...
        let tryAppend (propList: PropList) (context: PluginContext) (tagFinalizer: TagSource -> unit) (tagName: TagSource) (expr: Expr) : PropList =
            match expr with
            | Let({ Name = Helpers.StartsWith "returnVal" _ }, _, Sequential exprs) -> fromExpressions context exprs @ propList
    ...
    module Children =
        let appendFromExpr (currentList: Expr list) (context: PluginContext) (expr: Expr) =
            ...
            | DecisionTree(decisionTree, targets) ->
                DecisionTree(decisionTree, targets |> List.map (fun (target, expr) -> target, AST.transform expr context))
                :: currentList
            // router cases
            | Call(Get(IdentExpr _, FieldGet _, Any, _), { Args = args }, _, _) ->
                ...
                | [ Tags.LibraryTagImport(imp, _) ] ->
                    let newExpr = TagInfo.NoChildren(LibraryImport imp, [], None) |> TagInfo.transform context
                    newExpr :: currentList
                ...
                | [ Let(_,
                    ...
                    let newExpr =
                        TagInfo.Combined(LibraryImport imp, exprs, callInfo, None) |> TagInfo.transform context
            ...
            | Let({ Name = name
                  ...
                  _,
                  _) when
                ((name.StartsWith("returnVal") && (fullName.StartsWith("Oxpecker.Solid") || fullName.StartsWith("Partas.Solid")))
                 || (name.StartsWith("element") && (fullName.StartsWith("Oxpecker.Solid") || fullName.StartsWith("Partas.Solid"))))
                |> not
                ->
                match range with
                | Some range ->
        ...
        let transform (context: PluginContext) (children: Expr list) =
            GetSet.transformCollection children context |> Helpers.Baked.exprListToExpr |> Helpers.wrapChildrenExpression
    ...
    module ElementBuilder =
        ...
        let fromTagInfo (context: PluginContext): TagInfo -> ElementBuilder =
            ...
            | WithChildren(tagName, callInfo, range) ->
                ...
                let properties = callInfo.Args |> List.fold (fun state -> Attributes.tryAppend state context tagFinalizer tagName ) []
                let children = callInfo.Args |> List.fold (fun state -> Children.appendFromExpr state context) [] 
    ...
    module TagInfo =
        let transform (context: PluginContext) = ElementBuilder.fromTagInfo context >> ElementBuilder.transform context
    ...
    module GetSet =
        let (|Setter|_|) =
            ...
            | Call(_,
                ...
                | Helpers.StartsWithTrimmed "HtmlTag.set_" setTarget ->
                    Some(identExpr, setTarget, args)
                | _ -> None
            | _ -> None
        ...
        let private transform (context: PluginContext) = function
            | Setter(identExpr, setTarget, args) ->
                (setTarget, args)
                |> PluginContext.addSetter context
                None
            | Getter(targetName, getTarget)
            | TypeCast(Getter(targetName, getTarget), _) ->
                PluginContext.addGetter context getTarget
                Helpers.Baked.propGetter(PluginContext.getterName, getTarget)
                |> Some
            | _ as expr ->
                expr |> Some
    ...
    let transform (expr: Expr) (context: PluginContext) =
        ...
        | Tags.LibraryTagImport(imp, range) -> TagInfo.NoChildren(LibraryImport imp, [], range) |> TagInfo.transform context
        ...
        | Let(_, Tags.LibraryTagImport(imp, range), Tags.ConstructorBuilder(_, callInfo, _)) ->
            TagInfo.WithChildren(LibraryImport imp, callInfo, range) |> TagInfo.transform context
        | Let(_, Let(_, Tags.LibraryTagImport(imp, range), Sequential exprs), Tags.ConstructorBuilder(_, callInfo, _)) ->
            TagInfo.Combined(LibraryImport imp, exprs, callInfo, range) |> TagInfo.transform context
