...
Partas.Solid would compile the component function in `Extensions.fs.jsx`, but any usage of the type constructor would instead
refer to `MyComponent.fs.jsx` which would have no function of the same name declared.
...
```fsharp tab="Key1" title="F#"
let finalisedClass =  props.class' + " bg-primary" 
```
```jsx tab="Key1" title="JSX"
...
See [Tag Values](/partas-solid/tag-values)
