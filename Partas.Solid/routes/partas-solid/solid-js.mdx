...
```fsharp
type Dynamic<'T>() = interface HtmlElement
```
...
```fsharp
val componentAsString: string
```

Overload to `component'` accepting a string for native components.
...
```fsharp
type NoHydration() = interface FragmentNode
```
...
:::warning[Render Function]

Alternate typed syntax for using a render function:
...
val revealOrder: SuspenseList.RevealOrder
```
...
```fsharp
val fallback: ErrorBoundary.Fallback
```

```fsharp
val plainFallback: HtmlElement
```

Overload for `fallback` which accepts a simple `HtmlElement` when you don't need to introspect the error or reset
the boundary.
...
:::warning[Working with Functions]
To store a function, you will have to keep in mind that subsequent *setting* of the signal will have to be
in function form (except during initial assignment):
...
```fsharp
let createMemo(value: 'T -> 'T, initialValue: 'T): Accessor<'T>
```
