...
A callback function for `createRootPool`. Called when a new root is created.

|Param|Desc|
|---|---|
|`arg`|An accessor that returns the argument passed to the `RootPoolFunction`|
|`active`|An accessor that returns the active state of the root. If `false`, the root is not being used and is waiting in the pool to be reused|
|`dispose`|A function that disposes the root and prevents it from being reused|
