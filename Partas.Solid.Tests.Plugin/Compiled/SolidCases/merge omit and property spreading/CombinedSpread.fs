...
[<Erase>]
type SimpleCombination() =
    interface RegularNode

    [<SolidTypeComponent>]
    member this.CombinedSpread =
        this.class' <- "ClassDefault"

        div () {
            "Some text"
            span (class' = this.class') { "More text" }
            div().spread this { "All but class have been spread into this tag!" }
...
type Button() =
    ...
    [<SolidTypeComponent>]
    member someProps.constructor =
        button(
            class' =
                button.variants (
                    {| size = someProps.size
                       variant = someProps.variant |}
                )
        )
            .spread
            someProps
